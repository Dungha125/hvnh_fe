...
const contestId = localStorage.getItem('contest_id') || route.params.id
...
const sortedUsers = computed(() => {
  const users = contest.value?.users?.data || []
  const scores = contest.value?.scores || {}

  return [...users].sort((a, b) => {
    const getTotal = (user) =>
      problemIds.value.reduce(
        (sum, pid) => sum + (scores[user.username]?.[pid] ?? 0),
        0
      )

    return getTotal(b) - getTotal(a) // Sắp xếp giảm dần
  })
})
