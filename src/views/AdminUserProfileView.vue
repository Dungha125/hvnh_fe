...
const showEditor = async (id) => {
  ...
  try {
    ...
    console.error('Lỗi khi mở bài làm:', error);
    message.error('Không thể mở bài làm này!');
  }
...
const fetchUserCourses = async () => {
  try {
    ...
    if (response.data.code === 200) {
      ...
      let stt = 1;
      courses.length = 0; // Xóa dữ liệu cũ
      data.forEach(course => {
        courses.push({
          stt: stt++,
          id: course.id,
...
const fetchUserSubmissions = async () => {
  try {
    ...
    const response = await axios.get(`solutions?username=${currentUser.username}`, {
      headers: {
        'Authorization': `Bearer ${adminToken}`
      }
    });
    ...
    if (response.status === 200 && response.data.code === 200) {
      ...
      status.length = 0; // Xóa dữ liệu cũ
      ...
      <div class="part-left">
        <div class="body-header">
          <div class="admin-view-header">
            <a-button type="primary" @click="returnToAdminUsers" style="margin-right: 10px">
              <ArrowLeftOutlined /> Quay lại
            </a-button>
            <h2>Hồ sơ người dùng (Chế độ quản trị)</h2>
          </div>
            ...
            <div class="info-container" ref="infoContainerRef">
              <div
                ...
                <a-avatar
                    ...
                    :src="currentUser && currentUser.profile_picture ? currentUser.profile_picture : 
                            currentUser ? `https://ui-avatars.com/api/?name=${getAvatarName(currentUser.last_name + ' ' + currentUser.first_name)}` : ''"
                    alt="Avatar"
                />

                <p style="margin-top: 10px; font-weight: bold; font-size: 130%">
                  {{ currentUser ? currentUser.last_name + ' ' + currentUser.first_name : 'Đang tải...' }}</p>
              </div>

              <div class="admin-badge">
                <a-tag color="#A7453C">Đang xem với quyền quản trị</a-tag>
              </div>
              ...
              <div class="detail-container">
                ...
                <p>
                  <span style="font-weight: bold">
                      <WomanOutlined v-if="currentUser && currentUser.gender === 1"/>
                      <ManOutlined v-else-if="currentUser"/>
                      Giới tính:
                  </span>
                  {{ currentUser && currentUser.gender === 0 ? 'Nam' : currentUser && currentUser.gender === 1 ? 'Nữ' : '' }}
                </p>
